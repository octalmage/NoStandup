...
$(".titleAndTag").append("<span style='float:right; font-weight:bold; font-size:14px; padding-top:15px; padding-right: 10px;'>" + nostandupswitch + "</span><span style='float:right; font-weight:bold; font-size:14px; padding-top:25px; padding-right: 10px;'>Standup:</span>");
...
function ShowStandup()
{
	...
	for (id in hidden)
	{
		...
		//$("#dbs" + hidden[id] + "_0").children().show();
		$("#dbs" + hidden[id] + "_0").parent().show();
...
{
	//$(".nostanduptemp").remove();
	$(".boxShot:not(.nostanduptemp):not(:hidden)").each(function (index)
	{
		id = $(this).attr("id").match(/\d......./i)[0]; //I fail at regex, I know this can be done better. 
		if (standup.indexOf(id) > -1)
		{
			...
			$(".agMovieGallery").append("<div class='nostanduptemp agMovie agMovie-lulg mrNR'><span style='border-color: #e1e1e1;background-color:#e1e1e1;' class='nostanduptemp boxShot boxShot-166 queueable hoverPlay bobbable'><img class='boxShotImg hideBobBoxshot' style='display:none' src=''></span></div>");
			//$(this).children().hide()
